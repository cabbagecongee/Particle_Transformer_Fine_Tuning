...
metrics_path = os.path.join(SAVE_DIR, "training_metrics_pretrain_1_more_data.csv")
...
train_files = filepaths[:int(0.4*n)]
val_files = filepaths[int(0.4*n):int(0.55*n)]
...
for epoch in range(EPOCHS):
  ...
  train_loss_all = accelerator.gather(torch.tensor(train_loss_acum, device=accelerator.device)).sum().item()
    ...
    if accelerator.is_main_process:
        ...
        if avg_val_loss < best_val_loss:
            ...
            accelerator.save(
                ...
                os.path.join(SAVE_DIR, f"pretrain_1_more_data_best_loss_epoch{epoch+1}.pt")
            )
        ...
        if val_accuracy > best_val_acc:
            ...
            accelerator.save(
                accelerator.unwrap_model(model).state_dict(),
                os.path.join(SAVE_DIR, f"pretrain_1_more_data_best_acc_epoch{epoch+1}.pt")
            )
...
if accelerator.is_main_process:
    plot_path = os.path.join(SAVE_DIR, "pretrain_1_more_data_accuracy_plot.png")
...
if accelerator.is_main_process:
    ...
    plt.grid(True)
    plt.tight_layout()
    plot_path = os.path.join(SAVE_DIR, "pretrain_1_more_data_loss_plot.png")
    plt.savefig(plot_path)
