...
metrics_path = os.path.join(SAVE_DIR, "training_metrics_base25%.csv")
...
val_files = filepaths[int(0.25*n):int(0.4*n)]
...
total_steps = num_iterations * EPOCHS
    ...
    if accelerator.is_main_process:
        ...
        if avg_val_loss < best_val_loss:
            ...
            accelerator.save(
                ...
                os.path.join(SAVE_DIR, f"base25%_best_loss_epoch{epoch+1}.pt")
            )
        ...
        if val_accuracy > best_val_acc:
            ...
            accelerator.save(
                accelerator.unwrap_model(model).state_dict(),
                os.path.join(SAVE_DIR, f"base25%_best_acc_epoch{epoch+1}.pt")
            )
...
if accelerator.is_main_process:
    plot_path = os.path.join(SAVE_DIR, "base25%_accuracy_plot.png")
...
if accelerator.is_main_process:
    ...
    plt.grid(True)
    plt.tight_layout()
    plot_path = os.path.join(SAVE_DIR, "base25%_loss_plot.png")
    plt.savefig(plot_path)
