#changed model to 8 layers and training split is 25% validation 5%
...
metrics_path = os.path.join(SAVE_DIR, "training_metrics_base_4.csv")
    ...
    if accelerator.is_main_process:
        ...
        if avg_val_loss < best_val_loss:
            ...
            accelerator.save(
                ...
                os.path.join(SAVE_DIR, f"base_4_best_loss_epoch{epoch+1}.pt")
            )
        ...
        if val_accuracy > best_val_acc:
            ...
            accelerator.save(
                accelerator.unwrap_model(model).state_dict(),
                os.path.join(SAVE_DIR, f"base_4_best_acc_epoch{epoch+1}.pt")
            )
...
if accelerator.is_main_process:
    plot_path = os.path.join(SAVE_DIR, "base_4_accuracy_plot.png")
...
if accelerator.is_main_process:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(SAVE_DIR, "base_4_loss_curve.png"))
    plt.show()

    plot_path = os.path.join(SAVE_DIR, "base_4_loss_plot.png")
    plt.savefig(plot_path)
    plt.show()
